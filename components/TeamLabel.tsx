...
export default function TeamLabel({
  ...
  const teamName = name ?? resolved?.name ?? fallbackLabel ?? 'Pendiente de clasificar';
  ...
  const textSize = size === 'sm' ? 'text-xs md:text-sm' : size === 'lg' ? 'text-base md:text-lg' : 'text-sm md:text-base';
  const badgeSize = size === 'sm' ? 'h-5 w-5 text-xs' : size === 'lg' ? 'h-8 w-8 text-base' : 'h-6 w-6 text-sm';

  return (
    <span className={`inline-flex max-w-full items-center gap-2 ${wrapper} ${className}`}>
      <span className={`inline-flex shrink-0 items-center justify-center rounded-full border border-wc-border bg-wc-background/95 ${badgeSize}`}>
