...
export const recalculateScores = onRequest({ cors: true }, async (req, res) => {
  ...
  if (finalMatch && finalMatch.winnerAfter90Or120 && finalMatch.winnerAfter90Or120 !== 'draw') {
    realChampion = finalMatch.winnerAfter90Or120 === 'home' ? finalMatch.homeTeam : finalMatch.awayTeam;
  }

  const predsSnap = await db.collection('predictions').where('tournamentId','==',tournamentId).get();
  ...
  for (const d of predsSnap.docs) {
    ...
    const result = computeScore(pred, matches, realChampion);
    const part = partsById.get(pred.participantId);
    const ref = db.doc(`scores/${pred.participantId}_${pred.tournamentId}`);
    batch.set(ref, {
      id: `${pred.participantId}_${pred.tournamentId}`,
      participantId: pred.participantId,
      tournamentId: pred.tournamentId,
      totalPoints: result.totalPoints,
      breakdown: result.breakdown,
      fullName: part?.fullName || 'Participante',  // denormalizado para ranking público
      updatedAt: Date.now(),
    });
    n++;
  }
