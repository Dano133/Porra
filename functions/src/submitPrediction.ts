...
export const submitPrediction = onRequest({ cors: true }, async (req, res) => {
  ...
  try {
    ...
    if (!settingsSnap.exists) {
      res.status(500).json({ error: "Settings no inicializados" });
      return;
    }
    ...
    const pSnap = await db
      ...
      .get();
    if (pSnap.empty) {
      res.status(404).json({ error: "No estás registrado." });
      return;
    }
    const participant = pSnap.docs[0].data() as any;
    if (participant.status === "blocked") {
      res.status(403).json({ error: "Cuenta bloqueada." });
      return;
    }
    ...
    if (existing.exists) {
      ...
      if (cur.status === "locked" || cur.status === "cancelled" || closed) {
        res
          .status(403)
          .json({ error: "El plazo está cerrado o tu porra está bloqueada." });
        return;
      }
    }
    ...
    await db.doc(`predictions/${id}`).set(
      {
        ...
        pichichiPrediction: data.pichichiPrediction,
        submittedAt:
          newStatus === "submitted"
            ? now
            : (existing.data() as any)?.submittedAt,
        updatedAt: now,
    ...
    if (newStatus === "submitted") {
      ...
      try {
        await sendMail(
          participant.email,
          "Porra recibida",
          confirmationTemplate(participant.fullName, siteUrl),
        );
      } catch (e) {
