        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setS({
                ...s,
                predictionDeadline: new Date(e.target.value).getTime(),
              })
            }
          />
        </div>
        <Toggle
          label="Ranking público activado"
          value={s.publicLeaderboardEnabled}
          onChange={(v) => setS({ ...s, publicLeaderboardEnabled: v })}
        />
        <Toggle
          label="Emails automáticos activados"
          value={s.emailUpdatesEnabled}
          onChange={(v) => setS({ ...s, emailUpdatesEnabled: v })}
        />
        <Field
          label="Nombre remitente"
          value={s.senderName}
          onChange={(v) => setS({ ...s, senderName: v })}
        />
        <Field
          label="Email remitente"
          value={s.senderEmail}
          onChange={(v) => setS({ ...s, senderEmail: v })}
        />
        <Field
          label="Torneo actual (ID)"
          value={s.currentTournamentId}
          onChange={(v) => setS({ ...s, currentTournamentId: v })}
        />
        <button
          onClick={save}
          className="min-h-11 rounded-md bg-wc-primary px-4 py-2 text-white"
        >
          Guardar
        </button>
